...
usage(){
    printf '%s\n\n' \
        ...
        "  4) $0 <run_id> variance <thread> # report variance under a thread" \
        "  5) $0 <run_id> single <qid>      # run a single query" \
        "  6) $0 ls                         # show run IDs" \
        "Note:" \
        "  * default: mysf=1, mykind=\"opt\", myavx=\"yes\""
    exit 1 
}
...
run_build(){
    ...
    do
        ...
        debug="-DNO_PROFILE_INFO" myavx=${myavx} ./make.sh ${kind} ${id} &>> ${log_file}
    done
}
...
run_build_time(){
    ...
    do
        ...
        (set -x &&
            ...
            time make q${id} sys=${machine} debug="-DNO_PROFILE_INFO" myavx=${myavx} report="-ftime-report") &>> ${log_file}
    done
}
...
run_single(){
    id=$1
    (set -x && debug="-DNO_PROFILE_INFO" myavx=${myavx} ./make.sh ${mykind} ${id})
    for thread in ${threads[@]}
    do
...
run_report_variance(){
    ...
    local log_file="${log_folder}/${mykind}_sf${mysf}_t${thread}.txt"
    echo "Fetch execution time from ${log_file}"
    cat ${log_file} | grep "Run with" | cut -d'|' -f 2 | python util/report-variance.py
}
...
if [ $# -eq 1 ]; then
    ...
    if [ $2 = "report" ]; then
        set_log $1
        run_report $3
    elif [ $2 = "variance" ]; then
        set_log $1
        run_report_variance $3
