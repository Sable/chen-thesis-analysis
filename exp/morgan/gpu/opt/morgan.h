...
static I horse_morgan(V *h_rtn, V n, V x, V y){
    ...
    if(isNaive){
        ...
        if(!isGPU){
            ...
            E time_kernel = 0;
            tic();
#pragma acc data copyin(sx_ptr[0:r0],sy_ptr[0:r0],sxy_ptr[0:r0],sx2_ptr[0:r0],sy2_ptr[0:r0],n0) copyout(z_ptr[0:r0])
            {
                tic();
#pragma acc parallel loop
                for(L i=0; i<r0; i++){
                    z_ptr[i]=(sxy_ptr[i]/n0-sx_ptr[i]*sy_ptr[i]/pow(n0,2))/(
                            sqrt(fabs(sx2_ptr[i]/n0+pow(sx_ptr[i]/n0,2)))*
                            sqrt(fabs(sy2_ptr[i]/n0-pow(sy_ptr[i]/n0,2))));
                }
                // time_toc("Kernel execution time (ms): %g\n", elapsed);
